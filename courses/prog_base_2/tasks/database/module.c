...
#include "C:\Users\DrLove\Desktop\database\sqlite3.h"
#include "module.h"
...
void db_patient_insert(db_t * self, patient_t * patient){
    ...
    const char *sqlInsertCommand = "INSERT INTO Patient ('Name', 'Surname', 'Diagnosis', 'Days', 'Importance', 'Birthdate') VALUES (?,?,?,?,?,?);";
    int returnCode = 0;
    ...
    sqlite3_bind_int(stmt, 3, patient->diagnosis);
...
patient_t * db_patient_get(db_t * self, int id){
    ...
    toGet->diagnosis = sqlite3_column_int(stmt, 3);
...
int db_filter(db_t * self, double days, int diagnosis, patient_t * patientSet, int setMaxLen){
    sqlite3_stmt *stmt = NULL;
    const char *sqlCommand = "SELECT * FROM Patient WHERE Days > ? AND Diagnosis == ?;";
    int returnCode = 0;
